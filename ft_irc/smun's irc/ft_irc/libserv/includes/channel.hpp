...
/*   Updated: 2022/03/30 14:25:07 by seungyel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
private:
     ...
     *
     * kqueue의 이벤트는 udata 라고 부르는 포인터를 가질 수 있습니다. 
	 * 이 udata는 'U'ser 'Data'의 줄임말이며,
     ...
     * 따라서 accept 이벤트 역시도 다른 read 및 write 이벤트와 하나의 로직으로 묶기 위해서는, 
	 * 연결을 수락할 listen 소켓 역시도 일반적인 Context 클래스로 관리될 필요가 있습니다.
     */
    Context*    _listenContext;

    /**
     * 시그널 등으로 서버가 중단이 요청될 경우, 서버의 이벤트 루프를 중단시키기 위한 스위치 입니다.
     * 그렇게 구현하려면 EVFILT_USER 이벤트가 필요하고, 그러한 처리가 추가적으로 필요하나, 
	 * 과제의 필수 여부는 아직 확실하지 않습니다.
     */
    bool        _killed;
     ...
     * 소켓이 올바르게 동작하지 않을 가능성이 생기기 때문입니다. 따라서 Session 클래스는 
	 * 복사 생성자 및 대입 연산자를 허용하지 않고 있습니다.
     * 하지만 std::map 에 값을 넣기 위해서는 대입 연산자 또는 복사 생성자가 필요하게 되는 모순을 갖게 됩니다.
     * 따라서 이러한 모순을 해결하기 위해, 공유 포인터를 사용하여 Session 클래스가 필요할 때 
	 * 올바르게 소멸자가 호출되며 삭제될 수 있도록 제어합니다.
     *
     ...
     * 새로운 세션 인스턴스를 만든 후 세션 맵에 넣고, 해당 소켓에 대해 통신을 할 수 있게 
	 * IO이벤트를 kqueue에 등록합니다.
     *

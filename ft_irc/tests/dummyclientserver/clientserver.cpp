...
conn_t* connect_tcp(const char* serverIP, const char* serverPort) {
    ...
    fcntl(serverconn->sockfd, F_SETFL, O_NONBLOCK);
...
int send_tcp(conn_t* connxn, const char* buffer, unsigned int bytesToSend) {
	int ret;
	int retVal = 0;
	...
	while (retVal < (int)bytesToSend) {
		ret = send(connxn->sockfd, buffer+retVal, bytesToSend-retVal, 0);
		if( ret <= 0 ) {
...
int sendData(conn_t* connxn, const char* buffer, unsigned int bytesToSend) {
	int retVal = send_tcp(connxn, buffer, bytesToSend);
	if (retVal <= 0) {
		disconnect_tcp(connxn);
	}
	return retVal;

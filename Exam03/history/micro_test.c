...
void filled_rect(t_zone *zone, t_circle rect)
{
    int i = 0;
    int j;
    ...
    while (i < zone->height)
    {
        ...
        while (j < zone->width)
        {
            ...
            {
                zone->data_arr[j + i*zone->width] = rect.char_to_print;   
            }
            j++;
        }
        i++;
...
void empty_rect(t_zone *zone, t_circle rect)
{
    int i = 0;
    int j;
...
int get_circles_data(t_zone *zone, FILE *op_file)
{
    ...
    circle.type = 0;
    following_char = '\0';
    while((ret_code = fscanf(op_file, "%c %f %f %f %f %c", &circle.type, &circle.x, &circle.y, &circle.xd, &circle.yd, &circle.char_to_print)) == 6)
    {
        ret_code = fscanf(op_file, "%c", &following_char);
        if (ret_code == EOF || (ret_code == 1 && following_char == '\n'))
        {
            if (circle.type == 'r')
                filled_rect(zone, circle);
            if (circle.type == 'R')
                empty_rect(zone, circle);
        }
        else
            return (-1);
    }
    ...
    if (fscanf(op_file, "%d %d %c", &zone->width, &zone->height, &zone->char_to_print) == 3)
    {
        ...
        if (ret_code == EOF || ret_code == 1 && following_char == '\n')
        {
            if (zone->width <= 0 || zone->height > 300 || zone->height <= 0 || zone->width > 300)
                return (-1);
            zone->data_arr = (char*)malloc(sizeof(char) * zone->height * zone->width);
